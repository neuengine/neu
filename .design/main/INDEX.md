...
| [l1-multi-repo-architecture.md](specifications/l1-multi-repo-architecture.md) | Repository split architecture: pkg-based boundary between engine and editor | Stable | concept | 1.5.0 |
| [l2-multi-repo-architecture-go.md](specifications/l2-multi-repo-architecture-go.md) | Go impl: pkg/editor interfaces + pkg/protocol JSON wire messages, //go:build editor scoping, AST/go-list architecture-guard tests (Implements: l1-multi-repo-architecture) | Stable | go | 0.2.0 |
...
| File | Description | Status | Layer | Version |
| :--- | :--- | :--- | :--- | :--- |
| [l1-definition-system.md](specifications/l1-definition-system.md) | JSON declarative layer: UI, scenes, flows, templates — data-driven bridge | Stable | concept | 0.5.0 |
| [l2-definition-system-go.md](specifications/l2-definition-system-go.md) | Go impl: Kind-discriminated Envelope, DefinitionLoader with total validation (struct + TypeRegistry refs + include-DAG DFS) so instantiation is infallible, Command-only interpreters, DefinitionInstance hot-reload via AssetEvent, extensible ActionRegistry, reuses DynamicScene codec + NextState (Implements: l1-definition-system) | Stable | go | 0.1.0 |
...
| [l2-definition-integration-go.md](specifications/l2-definition-integration-go.md) | Go impl: stable pkg/editor DefinitionEditorInterface (interface-only, multi-repo INV-3) + internal/definition order-based serialization (explicit Order, missing→default, Content/Default depth) + EntitySerializer Pre/Post hooks (Implements: l1-definition-integration) | Draft | go | 0.1.0 |
| [l1-window-system.md](specifications/l1-window-system.md) | Window management, multi-window, platform abstraction | Stable | concept | 0.1.0 |
| [l2-window-system-go.md](specifications/l2-window-system-go.md) | Go impl: Window component as ECS entity, diff-driven sync via change-detection, deferred create/destroy commands, main-thread WindowBackend, headless CI backend, RawWindowHandle→render surface (Implements: l1-window-system) | Stable | go | 0.1.0 |
| [l1-diagnostic-system.md](specifications/l1-diagnostic-system.md) | Diagnostics, profiling, gizmos, error codes, debug overlay | Stable | concept | 0.1.0 |
| [l2-diagnostic-system-go.md](specifications/l2-diagnostic-system-go.md) | Go impl: DiagnosticsStore + fixed-cap RingBuffer (0-alloc Push, deterministic avg), hasAnyReader zero-cost gate, pooled gizmo RenderFeature, Last-schedule-only, build-tag profiling spans, slog per-module filter, error codes via l2-error-core-go (Implements: l1-diagnostic-system) | Stable | go | 0.1.0 |
| [l1-ui-system.md](specifications/l1-ui-system.md) | Layout engine, interaction, text, widgets, styling | Stable | concept | 0.2.0 |
| [l2-ui-system-go.md](specifications/l2-ui-system-go.md) | Go impl: Style component + Val union, flexbox solver with dirty-subtree relayout (Changed[Style]), PreUpdate Interaction hit-test + MouseFilter + focus neighbors, FontAtlas glyph cache over render-core DynamicAtlas, UiFeature compositing last + ZIndex, render-only OffsetTransform (Implements: l1-ui-system) | Stable | go | 0.1.0 |
...
- **Total Specifications**: 112 (65 L1 concept + 45 L2 Go + 1 test + 1 tool) | Stable: 76 | RFC: 0 | Draft: 36
...
- **Last Stabilization:** 2026-06-01 — **2 governance specs promoted Draft → Stable** (`/magic.task` Pre-Planning Stabilization, user-scoped to governance-only): `l1-compatibility-policy` 0.3.0 → 0.4.0 (fully realized by `pkg/version` SemVer/Go-matrix + `cmd/apidiff` breaking-gate + CI) and `l1-examples-framework` 0.4.0 → 0.5.0 (realized by `cmd/examplecheck` mechanical-validation + `_template`/README conventions + `goldens.json`; `cmd/showcase/` deferred by the spec itself, not overclaim). Both are L1-only governance specs (no L2 children) → layer-clean, no broken Stable→Draft edge (C24 Skeptic). Canonical References populated on both. **Editor-feature specs held Draft** (cli/plugin-distribution/ai-assistant/ai-api/visual-graph-bridge are core-done with deferred App-integration surface — promoting would overclaim, cf. the asset-formats lesson). **Stable 74 → 76, Draft 38 → 36.** Prior: 2026-05-31 — **asset-formats narrowed + promoted Draft → Stable** (`l1-asset-formats` 0.1.0→0.2.0 + `l2-asset-formats-go` 0.1.0→0.2.0; `/magic.task` Option A). Both specs were narrowed to the implemented surface — the glTF 2.0 loader (value-based fan-out of scene/mesh/material/texture + deterministic `GltfAssetLabel`, INV-2/INV-4; 90.0% cov + `FuzzDecode` + `examples/gltf` ×20) + stdlib PNG/JPEG + PCM WAV — with HDR/DDS/KTX2/BMP/WebP/TGA/OGG/FLAC/MP3/AAC/fonts/`.scene.json` + glTF animations/skins/morph/KHR explicitly marked **Planned**, so the spec no longer overclaims. The L2 was also corrected to the shipped value-fan-out (`asset.Handle[T]` registration needs a `LoadContext` the real `AssetLoader.Load(r,settings)` lacks → deferred to App). Canonical References populated on both; INV-1 cross-package conflict-detection + INV-3 build-tag optional formats noted as planned hardening. Layer-clean (L1 promoted before L2). **Stable 72 → 74, Draft 40 → 38.** Prior: 2026-05-30 — **Phase 6 engine-core cohort: 12 specs promoted Draft → Stable** (`/magic.task` Pre-Planning Stabilization): 6 L1 + 6 L2 for error-core, platform-system, window-system, diagnostic-system, ui-system, definition-system. C29 P6 gate closed by T-6T06 (`examples/{config,window,diagnostic,ui}` hash-stable ×20; full 58-pkg suite green). All 6 L2 specs' **Canonical References populated** with on-disk-verified source + test + example files. Cohort delivered across 3 `/magic.run` passes with >80% coverage each (errs 89.3%, platform 100%, diag 94.6%/100%, window 89.3%/82.6%, ui 95.5%/89.8%, definition 88.5%). Deferred (not blocking promotion — analogous to render-core nopBackend / App-integration wiring): ECS-system glue (window sync, diagnostic overlay/built-ins, definition hot-reload, ui PreUpdate) + real TTF rasterizer (x/image/font ADR). **Stable 60 → 72, Draft 47 → 35.** Prior: **Multi-Repo Architecture ratified: `l1-multi-repo-architecture` Draft → Stable (v1.5.0) + `l2-multi-repo-architecture-go` Draft → Stable (v0.2.0)** (`/magic.spec`). Evidence (mirrors render-core ratification): the engine-side boundary `pkg/editor/` + `pkg/protocol/` is implemented and the architecture-guard suite — the C29 validator named by the L2 spec — is green: `TestEditorPkgIsContractOnly` (INV-3), `TestNoEditorImports` (INV-1), `TestNoPackageInit`/`TestProtocol_NoPackageInit` (INV-5), `TestProtocolStdlibOnly` (INV-4), `TestProtocolRoundTrip` (all 8 wire Kinds) + unknown-kind/extra-field forward-compat. Both Canonical References populated with on-disk sources + guard tests. L1 open questions 2–4 confirmed non-blocking for the engine-side contract (WebSocket = additive, editor versioning = editor-repo scope, no-op interfaces = deferred to first EditorPlugin). This **unblocks the P6 editor-layer L2 tier** (ai-assistant, plugin-distribution, ai-api-plugin, definition-integration, cli — all `//go:build editor`, all consume `pkg/editor` + `pkg/protocol`). **Stable 58 → 60, Draft 49 → 47.** Prior: **Phase 5 Content Systems: 8 specs promoted Draft → Stable** (`/magic.task` Pre-Planning Stabilization): 4 L1 + 4 L2 for audio, animation, tweening, 2d-rendering. C29 P5 gate closed by T-5T05 (46/46 pkgs PASS; `examples/{audio,animation,tweening,2d}` hash-stable ×20). The 4 promoted L2 specs' **Canonical References populated** with on-disk-verified source + test + example files (closing the P4-style empty-refs advisory debt for P5). **Held Draft: `l1-asset-formats` + `l2-asset-formats-go`** — the spec's headline glTF multi-asset fan-out (INV-4), `.scene.json` codec, and font loaders are unimplemented (only stdlib image + WAV loaders landed); no end-to-end example in the C29 gate. Not a headless-stub gap (cf. render-core nopBackend) but an absent subsystem → promotion would overclaim. Revisit when the glTF loader lands. **Stable 50 → 58, Draft 51 → 43.** Prior: **Render Core ratified: `l1-render-core` RFC → Stable + `l2-render-core-go` Draft → Stable** (`/magic.spec`). Evidence: Phase 4 complete (19/19) + C29 P4 gate closed by T-4T05; L1 Canonical References already filled, Q4/Q5 resolved, Q1–Q3 annotated non-blocking; L2 Canonical References populated with 11 verified source files + 2 conformance/isolation tests. Bootstrap fully deactivated for **all 10 P4 render specs** — Phase 4 = 10/10 Stable. Phase 5 gate ("Render Core Stable") now cleared. Prior: **P4 Render Pipeline: 8 specs promoted Draft → Stable** (4 L1 + 4 L2: mesh-and-image, materials-and-lighting, camera-and-visibility, post-processing) via Pre-Planning Stabilization — C29 P4 gate satisfied by T-4T05 (`examples/{3d,camera,shader}/` validated, 36/36 pkgs PASS, 0-alloc hot paths). Prior: P1 ECS (2 new L2: `l2-pool-go`, `l2-view-go`) promoted directly to Stable (L1 parent Stable + MVC + C9 Trust Mode). `l1-render-core` promoted Draft → RFC (v0.6.0: +`Destroy`, +handle layout, +Canonical References, Q4/Q5 resolved). Prior: 2026-05-18 — P3 Assets/Math/Concurrency (4 L1 + 4 L2: task, asset, scene, math) promoted Draft → Stable via Pre-Planning Stabilization (C29 P3 gate satisfied by T-3T05 — `examples/{async,asset,scene,math}/` green). Bootstrap deactivated for P3. Prior: 2026-05-17 — P2 Framework (6 L1 + 7 L2) promoted Draft → Stable (gate: `examples/ecs/framework/`; multi-repo l1/l2 stay Draft — RFC-gated, Exit Criterion via /magic.spec)
