...
**Based on:** .design/main/INDEX.md v2.44.0
**Based on RULES:** .design/RULES.md v1.10.0
**Status:** Active
**Mode:** Specs `Stable` **86/114**. **Phases 1–6 Done.** **Phase 7 (Advanced Core) STARTED 2026-06-04** — the **hot-reload + profiling slice** is decomposed (`l2-profiling-protocol-go` → T-7A01..04; `l2-hot-reload-go` → T-7J01..04; both `[Bootstrap]`, parallelizable, grounded on shipped Phase-6 infra). The multiplayer stack (networking/transport/replication/sync/rpc/diagnostics) + Phase 8 physics stay deferred Bootstrap (L1-only — author L2 via `/magic.spec` first). Phase 6 closed 2026-06-03 (C32 zero-dep ADRs, CHANGELOG L1). **Deferred-future:** glTF skins/morph, asset-formats/animation spec graduations. **Next:** `/magic.run "phase-7"` to execute the slice. A manifest version-bump is deferred to Plan Completion (all 8 phases), not per-phase.
...
**Active slice (decomposed — see phase-7.md):**

- [ ] **Profiling Protocol** ([l1-profiling-protocol.md](specifications/l1-profiling-protocol.md)) [L1]
  - [ ] **Profiling Protocol — Go** ([l2-profiling-protocol-go.md](specifications/l2-profiling-protocol-go.md)) [L2] [Bootstrap] → T-7A01..04
- [ ] **Hot Reload** ([l1-hot-reload.md](specifications/l1-hot-reload.md)) [L1]
  - [ ] **Hot Reload — Go** ([l2-hot-reload-go.md](specifications/l2-hot-reload-go.md)) [L2] [Bootstrap] → T-7J01..04

**Deferred multiplayer stack (L1-only — author L2 via `/magic.spec` before decomposing):**
