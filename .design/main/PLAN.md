...
**Based on:** .design/main/INDEX.md v2.43.0
**Based on RULES:** .design/RULES.md v1.9.0
**Status:** Active
**Mode:** Specs `Stable` **86/112**. **Phases 1–6 Done.** **Phase 6 CLOSED 2026-06-03** — all activated tasks Done (engine-core + App-integration + validation cohorts + tooling E/F/I/J/L/M + editor H/N/O/P), 5 editor-feature families promoted Draft→Stable, the font/ws ADRs decided zero-dep (rule **C32**), CHANGELOG L1 written, `phase-6.md` `status: Done`. **Deferred-future (not blocking):** glTF skins/morph, the asset-formats spec graduations (scene.json spawn + glTF animations now implemented). **Next:** **Phase 7 (Advanced Core)** planning via `/magic.task`. A manifest version-bump is deferred to Plan Completion (all 8 phases), not per-phase.
