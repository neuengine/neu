...
The Visual Graph System provides engine-side infrastructure for **node-based visual programming** — the runtime data model, interpreter, and public API that enable a GUI editor (in the external `editor` repository) to offer a Blueprint-style graph editing experience. The engine does not implement the visual editor itself; it exposes the "door" — the graph data model, execution engine, serialization format, and `pkg/editor/` interfaces — so that the editor (or any other consumer of the public API) can build a full node-graph UI on top.
...
The engine exposes interfaces under `pkg/editor/` that the external `editor`
