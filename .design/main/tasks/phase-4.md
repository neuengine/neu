...
- [ ] **T-4A01** — RID + command-queue server, `RenderBackend` interface, two-phase resource create, `resourceTracker` (refcount + deferred-delete). Files: `pkg/render/backend.go`, `internal/render/{server,resources}.go`. Spec: [l2-render-core-go.md](../specifications/l2-render-core-go.md) §Server/§Type Definitions. `[Bootstrap]`
  Verify: `go test -race ./internal/render/... -run 'TestServer|TestResourceTracker'` — concurrent Submit from N goroutines dedups onto render goro; zero-ref RID destroyed exactly one frame after last Release (not in-flight); `go vet ./...` clean; `go list -deps ./pkg/render | rg -v '^(internal/|render)'` → stdlib-only (C-003).
- [ ] **T-4A02** — `RenderGraph` (Kahn topological sort reusing scheduler DAG pattern, barrier insertion, cycle → `ErrRenderGraphCycle`), `RenderPass` interface, pin/unpin of pass I/O RIDs (INV-3). Files: `internal/render/graph.go`. `[Bootstrap]`
  Verify: `go test -race ./internal/render/... -run TestRenderGraph` — table-driven acyclic/cyclic fixtures (cycle ⇒ ErrRenderGraphCycle with node IDs); barrier list matches golden resource-transition fixture; pass referencing released RID fails build (`ErrResourceReleased`).
- [ ] **T-4A03** — `RenderSubApp` (own World+schedule, plug into App after main Update), `ExtractFn` registry, four-phase schedule (Collect/Extract/Prepare/Draw) with single-extract frame guard (INV-4). Files: `internal/render/{subapp,extract,phases}.go`. Requires: T-4A01. `[Bootstrap]`
  Verify: `go test -race ./internal/render/... -run 'TestExtractIsolation|TestFrameGuard'` — mutating main world during Extract leaves render-world snapshot unaffected; Extract invoked exactly once/frame, ordered Before Prepare; passes only in DrawStage.
