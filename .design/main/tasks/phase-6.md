...
- [ ] [T-6P04·rem.2] `graphDebugSyncSystem` (`PostUpdate`, editor-attached) draining `DebugStore.DrainEvents()` → emitting `pkg/protocol` graph IPC events (`GraphBreakpointHit`/`GraphExecutionTraceEvent`/`GraphRuntimeError`/`GraphLiveUpdate`) + the interpreter trace hook (record per-step frames into the DebugStore via an injected recorder, keeping `pkg/visualgraph` protocol-free).
