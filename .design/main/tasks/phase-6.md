...
- [~] [T-6N03·rem] In-process loader remainder — **split (discovery+approval done; lifecycle-driving pending)**:
  ...
  - [ ] **T-6N03b in-process lifecycle driving** (deferred): instantiate an approved plugin + drive `StateApproved → StateLoading → Build → StateActive`. Needs a **compile-time factory registry** (`PluginID → func() appface.Plugin` — Go can't load a plugin from a manifest string: no `.so`, no func-by-name) and, for the full `Ready/Finish/Cleanup`, an `appface.Plugin` **lifecycle extension** (today the interface is `Build` only) — a Phase-2 App-framework change. Folds naturally into **T-6N04** (the OOP loader shares the same lifecycle).
