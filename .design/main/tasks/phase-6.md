...
- [x] [T-6A03] Definition hot-reload — **DONE 2026-06-02** (split into 2 sub-tasks, both complete; the hot-reload vertical now runs end to end: dev file change → `asset.Reload` → `AssetEvent[Definition]` → this system → `InstanceStore.Reload`):
  - [x] **T-6A03.1 instantiation adapter + instance registry** *(**Done 2026-06-02**)* — `internal/definition/instantiate.go`: `worldSink` realizes `definition.CommandSink` against a World (SpawnEntity→`SpawnEmpty`; InsertComponent→inject-resolved `name→reflect.Type` + **JSON round-trip coercion** + `w.Insert` auto-register; SetParent→`hierarchy.ChildOf`); `construct` reuses `encoding/json` for map→typed coercion; `Instance{Entities, Actions}` + `InstanceStore{byPath}` with `Despawn`/`Reload` (the per-definition spawned-entity registry hot-reload needs). Decoupled via an injected `TypeResolver` (app wires typereg; tests inject a map); boundary mismatches collected, never silent. Flow **action execution deferred** (actions collected for the state-machine pass — instantiation only produces entities). **97.8% cov**; new package, no cycle. *(Discovery: T-6A03 sat on this whole unbuilt definition→World pipeline — neither the adapter nor a TypeRegistry-backed reflective construction existed; only the test `recordingSink`.)*
  - [x] **T-6A03.2 hot-reload system** *(**Done 2026-06-02**)* — `internal/definition/hotreload.go`: `NewHotReloadSystem` (PreUpdate) reads `EventReader[asset.AssetEvent[Definition]]` and on `Modified` re-decodes the path and calls `InstanceStore.Reload` (despawn prior + re-instantiate); `DefinitionLoader` (`asset.AssetLoader[def.Definition, struct{}]`) lets definitions participate in asset hot-reload; `HotReloadPlugin` wires the loader + `WatchAssetType[Definition]` + the InstanceStore resource + the system. **Design:** the AssetEvent is the *signal*; the new value comes from an **injected `DecodeFunc`** (app→VFS), sidestepping the asset store's async-decode timing — `internal/definition` stays decoupled from the asset internals; alloc-free `IsEmpty` gate (C-004); decode/realization errors slog-logged, never fatal. **96% pkg cov** (full system via `Run`, no-bus no-op, non-Modified skip, decode-error, loader, plugin both branches). *Caveat:* the `.json` loader extension collides with the scene loader (pre-existing AssetServer compound-extension limitation); Flow action *execution* still deferred (collected in `Instance.Actions`).
...
- [x] [T-6D04.1] UI interaction stack (font-free half) *(**Done 2026-06-03**)* — `internal/ui/system.go` `InteractionPlugin` (opt-in, NOT in DefaultPlugins) adds one `PreUpdate` `ui.Interaction` system that (1) `solveLayout` builds a `LayoutNode` tree from the `Style`-bearing entities (linked via `hierarchy.Children`, order preserved; roots = no UI-node parent), `Solve`s each root subtree against a `UIViewport` resource, and writes each node's computed `pkgui.LayoutRect` back; (2) `updateInteraction` hit-tests `CursorPosition` against those rects (painter Z-order, `MouseFilter` Stop/Pass/Ignore via the existing `HitTest`), writes each node's `Interaction` (`InteractionFor`), and on a fresh left-press moves the sole `Focused` marker (cleared on the prior holder via a `focusState` resource). **Key calls:** one combined system (not two) guarantees layout-precedes-hit-test without intra-schedule ordering assumptions; the layout pass hands laid rects to the interaction pass in-memory (no same-frame re-query); two-phase writes avoid query-mid-mutation. Built on the T-6D01..03 pure primitives (`Solve`/`HitTest`/`InteractionFor`/components) — the deferred App-wiring, mirroring T-6A03/T-6P04. **internal/ui 91.8% cov** (system.go: Build/solveLayout/isRoot/setFocus 100%, updateInteraction 92%; flat + hierarchy layout, hover/press/miss/MouseIgnore/no-cursor/empty, focus transfer, plugin build). default `go test ./...` **75 pkgs green**; vet + modernize clean; own files gofmt-clean; `-race` in CI (CGO absent locally). — `internal/ui/system.go`
- [ ] [T-6D04.2] Font-atlas glyph upload (← **T-6X04** font ADR) — `FontAtlas` glyph rasterization + upload into the UI interaction/render path. **Gated** on the `x/image/font` C-003 ADR (no stdlib TTF rasterizer); deferred until T-6X04 resolves. — `internal/ui/`

### Group 2 — Plugin distribution App-integration (Track N; ← Phase 2 App Framework)

- [x] [T-6N03·rem] In-process loader remainder — **split (all parts done)**:
  - [x] **T-6N03a discovery + capability-grant approval** *(**Done 2026-06-02**)* — `internal/plugin/discovery.go`: `Source` + `DirSource` (walks an `fs.FS` for `plugin.toml` → `ParseManifest`, collects bad manifests without aborting); `grants.go`: `ResolveGrants` folds `RequiredCaps` over their approval tiers (TierGranted auto / TierPrompt prompt-once-and-persist / TierAlwaysPrompt prompt-every-session-never-persist, INV-9) against a `GrantStore` + `PromptFunc`, plus `MemoryGrantStore` + `MarshalGrants`/`UnmarshalGrants` (deterministic JSON persistence); `install.go`: `DiscoverAndRegister` scans sources → resolves grants → `Manager.Register` (compat INV-2 + dup INV-5 → `StateApproved`), returning a `DiscoveryReport`. **91.4% pkg cov.** The "4 sources" are config (N `DirSource`s), not code.
  - [x] **T-6N03b in-process lifecycle driving** *(**Done 2026-06-02**, bundled with T-6N04)* — `internal/plugin/registry.go`: compile-time `RegisterFactory`/`LookupFactory` (`PluginID → func() appface.Plugin` — Go has no .so runtime lookup, factories must be registered at link time); `internal/plugin/inproc.go`: `LoadInProcess` (looks up factory, calls `Build(app)` then `Ready(app)` if `FullPlugin`, sets `StateActive`) + `FinishInProcess` (drives `Finish/Cleanup`, sets `StateDisabled`); `pkg/app/appface/lifecycle.go`: `FullPlugin` optional interface extending `Plugin` with `Ready/Finish/Cleanup` — checked via type assertion so existing plugins that only implement `Build` are unaffected; `pkg/app/app.go`: `lifecycle []FullPlugin` slice + `AddPlugin` enrolls `FullPlugin` implementors + `Run()` calls `Ready` after all Builds, `Finish/Cleanup` in reverse on exit (`slices.Backward`). **internal/plugin 93.6% cov**; `go test ./...` **74 pkgs green**, build/vet/modernize clean.
- [x] [T-6N04] OOP loader *(**Done 2026-06-02**)* — `internal/plugin/oop/supervisor.go`: `Supervisor` manages one OOP subprocess: `Spawn` (exec.Cmd + cwd-restrict via `cmd.Dir` + `spawnFromCmd` inner fn + `PluginInit`/`PluginHello` handshake with `handshakeTimeout`); `DriveLifecycle` (sends `PluginLifecycle{Phase}` + awaits `PluginLifecycleDone` within `ctx` — context cancellation → `MarkFailed`; crash/EOF → `MarkFailed`; INV-8); `monitorProcess` goroutine (waits on `cmd.Wait()`, calls `MarkFailed` on unexpected exit; host never panics); `MarkFailed`/`IsFailed`/`Done` idempotent failure signalling. `internal/plugin/oop/proxyplugin.go`: `ProxyPlugin` implements `appface.FullPlugin` (Build/Ready/Finish/Cleanup) — routes each phase to `sup.DriveLifecycle`; failure in any phase marks `StateFailed`, never panics (INV-8); `Cleanup` cancels context + calls `sup.Close()` + sets `StateDisabled`. `pkg/protocol/plugin.go`: `PluginInit`/`PluginHello`/`PluginLifecycle`/`PluginLifecycleDone`/`PluginError` wire messages + `LifecyclePhase` consts; `codec.go` updated with 5 new Decode cases. **internal/plugin/oop 90.6% cov** (subprocess integration tests via `TestMain`+re-exec pattern); `go test ./...` **74 pkgs green**, build/vet/modernize clean.

### Group 3 — AI assistant/API App-integration (SPLIT 2026-06-02 per Skeptic; ← N lifecycle + event bus + Track A)

- [x] [T-6H02·rem] Transports + context *(**Done 2026-06-02** — http + ContextProvider; ws still ADR-gated)* — `pkg/assistant/http.go`: `HTTPTransport` (stdlib `net/http`, C-003 — no dep) + `HTTPConnection` implementing the shared `Connection` contract. Stateless model maps onto Send-then-Receive: `Send` does the POST (`http.NewRequestWithContext` → ctx cancellation/timeout INV-5) + buffers the decoded reply; `Receive` returns the buffered reply (single-shot per Send, mirrors `MemConnection`). Response body capped at `DefaultHTTPMaxResponseBytes` (8 MiB, defence-in-depth). `pkg/assistant/context.go`: `ContextProvider` + `EditorContext` (5 L2 fields) + info types (`EntityInfo`/`SceneInfo`/`TypeRegistrySummary`/`CommandRecord`/`DiagnosticsSummary`); `GetEditorContext(caps)` populates only capability-permitted fields (ReadScenes → entities+scene+commands; ReadTypeRegistry → types; ReadDiagnostics → diag) and **never invokes an ungated source** (INV-2 read-side). Sources are **injected** (`ContextSources` func fields) so `pkg/assistant` stays decoupled from world/typereg/diag; nil accessor ⇒ zero value (partial wiring safe). Per L1 §4.9 only structural metadata is sourced (no field values / log text / .env) → no secret-leak path. **91.0% pkg cov (`-tags editor`)** (http round-trip + manager-Dispatch integration via `httptest`, non-2xx/bad-JSON/close/single-shot/cancel/transport-error; context full/filtered/diag-only/no-caps/nil-sources). default `go test ./...` **74 pkgs green** (assistant excluded by build tag), `go build -tags editor ./...` + vet + modernize clean. **ws transport** stays behind the **ws-dep ADR** (C-003, user decision); editor UI panels live in the editor repo. **Unblocks T-6O01·rem.** — `pkg/assistant/`
- [x] [T-6O01·rem] AI-API plugin lifecycle *(**Done 2026-06-03**)* — `pkg/plugins/aiapi/plugin.go`: `AIAPIPlugin` implementing `appface.FullPlugin` (the T-6N03b lifecycle extension) — `New()` factory (the manifest `[entry.in_process] factory`), `SetConfig` (loader injects user config from `PluginContext.Config()`), `Build` (publishes the `ServiceRegistry` as a World resource via `SetResource`), `Ready` (resolves `Select(cfg)` provider + `resolveSecret` credentials + seeds the INV-1 redactor, then `activate`s the service — **non-blocking + graceful**: unconfigured provider or missing key ⇒ logs + leaves service not-ready, the L1 §4.11 "Ready false until creds resolve" semantics expressed via the service flag since `FullPlugin.Ready` has no return), `Finish` (method-dispatch wiring deferred to T-6O03·rem), `Cleanup` (`deactivate`s so in-flight consumers see `ErrServiceNotReady`, not a torn-down provider). `pkg/plugins/aiapi/service.go`: `ServiceRegistry` (the L1 §4.12 service-registry pattern, mirrors `internal/audio.ServiceRegistry`) — `AIService` interface (`Complete`/`Stream`/`Ready`) + RWMutex-guarded provider + ready flag; `Complete`/`Stream` forward to the active provider or return `ErrServiceNotReady` (optional-access graceful degrade). `pkg/plugins/aiapi/{manifest.go,plugin.toml}`: embedded manifest (`//go:embed`) — single-line arrays for the SDK's minimal TOML-subset parser; **dogfood-tested** via `pkg/plugin.ParseManifest`+`Validate` (the embedded manifest is provably loadable). **88.4% pkg cov (`-tags editor`)** (manifest validates + ID/mode/factory/network.outbound-required INV-4; lifecycle ready/graceful-unconfigured/graceful-missing-creds/nil-config-reset/cleanup; service not-ready→`ErrServiceNotReady`, forward-to-provider, deactivate). default `go test ./...` **74 pkgs green**, `go build -tags editor ./...` + vet + modernize clean. No `internal/` import (host app wires `RegisterFactory(aiapi.PluginID, aiapi.New)`). **Unblocks T-6O03·rem.** — `pkg/plugins/aiapi/`
- [x] [T-6O03·rem] Streaming + dispatch *(**Done 2026-06-03**)* — `pkg/plugins/aiapi/stream.go`: `decodeSSE` — a pure (transport-free, unit-testable) OpenAI-style Server-Sent-Events decoder; reads `data: {json}` lines, delivers each content delta to a sink, stops at `[DONE]` / a `finish_reason`-set final chunk / a sink error / **ctx cancellation between chunks (INV-6)**; malformed chunk ⇒ `CodeParse`, cancel ⇒ `CodeCancelled` (INV-5); 1 MiB line cap (defence-in-depth). Wired `openAIProvider.Stream` to issue `stream: true` + decode SSE (replacing the `ErrUnsupported` stub; `oaiRequest.Stream bool json:"stream,omitempty"` is additive — `Complete` leaves it false so existing non-stream tests stay byte-stable). `pkg/plugins/aiapi/dispatch.go`: `Dispatcher` routes the **10 standard assistant methods** (`assistant.Method*` constants — aiapi implements the protocol, so it references the canonical source, acyclic edge) → a `methodSpec`-driven canonical request (system prompt + lenient `extractInput`: declared key → prompt/input/text → join string params), enforces the per-provider rate limit (INV-8 → `CodeRateLimited`+retry-after), records usage (INV-9); **chat streams** via the provider `Stream`, forwarding each delta to an **injected `StreamEmitter`** (the app wires it to the ECS bus — an `AssistantStream` event readable next frame via T-6X01's swap; the plugin stays bus-decoupled) and accumulating the full text into the response. `pkg/plugins/aiapi/diagnostics.go`: `aiapi/tokens_input|tokens_output|cost_usd` diagnostics + `UsageRecorder` interface + `diagRecorder` (diag-backed; `Push` is **zero-cost until a reader subscribes**, INV-1) + `RegisterDiagnostics`. **89.0% pkg cov (`-tags editor`)** (SSE multi-chunk/[DONE]/keepalive/malformed/cancel/sink-error/scan-error + provider Stream httptest success/500/transport-err; dispatch unknown/not-ready/complete+usage/chat-stream-emitter/rate-limit/provider-error/extractInput; diag register/record/zero-cost-no-reader/skip-zero-cost). default `go test ./...` **74 pkgs green**, `go build -tags editor ./...` + vet + modernize + gofmt clean. **Deferred:** Command emission for `generate_*` world-mutation (← world integration), streaming provider usage (provider-dependent). **Unblocks T-6O05·rem.** (← T-6O01·rem) — `pkg/plugins/aiapi/`
- [x] [T-6O05·rem] Providers + parity *(**Done 2026-06-03** — closes Track O + Group 3)* — `pkg/plugins/aiapi/provider_anthropic.go`: Anthropic Messages API ↔ canonical (system prompt **hoisted** to the top-level `system` field, required `max_tokens` from the canonical `max_output_tokens` param or default 1024, `anthropic-version` header, content-block concatenation). `provider_gemini.go`: Gemini generateContent ↔ canonical (`systemInstruction` + `contents`/`parts`, **role mapping** assistant→model, `:generateContent` path, `usageMetadata` tokens). `provider_local.go`: one-line `RegisterProvider("local", newOpenAI)` — local servers (ollama/llama.cpp/vLLM) are OpenAI-compatible, so zero extra wire code + full native streaming, just a different base URL (L1 §2). All three register via `init()` → `Select` resolves all four. **DRY refactor:** extracted `httpclient.go` `postJSON` (POST + status→coded-error mapping INV-5 + read, one place) and refactored `openAIProvider.Complete` onto it (behaviour-preserving — existing httptest tests pass); `completeAsStream` adapts Anthropic/Gemini to the `Stream` contract (one Complete → single final chunk) so chat works through any provider (native per-provider SSE is a refinement). `parity.go`: `CheckParity`/`RunParity` (INV-7) — asserts a provider's canonical result is identical in-process vs across the **OOP JSON wire boundary** (`completeOverWire` marshals the request→subprocess + response→back through the same `pkg/protocol` JSON crossing); a deterministic `FakeProvider` makes the two comparable. **Honest scope:** full subprocess-RPC parity (driving a provider over a spawned `pkg/protocol` connection) lands when OOP **method** dispatch is built — the OOP loader (T-6N04) drives lifecycle only; this proves the canonical-boundary half (the INV-7 design claim) today. Shared `messageText`/`intParam` helpers added to `canonical.go`. **89.3% pkg cov (`-tags editor`)** (anthropic build/parse/HTTP/version-header/stream-fallback; gemini build/parse/role-map/HTTP/stream; local OpenAI-compat select; postJSON 401/header; parity fake/error/wire-roundtrip; all-four-registered). default `go test ./...` **74 pkgs green**, `go build -tags editor ./...` + vet + modernize + gofmt clean. **Track O complete (T-6O01..05).** — `pkg/plugins/aiapi/`

### Group 4 — Visual-graph editor wiring (Track P; ← App schedule + visualgraph interpreter + protocol graph IPC)

- [x] [T-6P04·rem.1] Concrete `GraphEditorPlugin`/`NodeRegistryQuery`/`GraphDebugger` impls + `DebugStore` *(**Done 2026-06-03**)* — new `internal/grapheditor/` (untagged — impls are registered at runtime when an editor attaches, not behind a build tag) maps the headless `pkg/visualgraph` runtime onto the editor-local DTOs without `pkg/editor` ever importing engine internals (guard test `TestEditorPkgIsContractOnly` stays green). `registryquery.go`: `RegistryQuery` (editor.NodeRegistryQuery) — `vg.NodeDescriptor/PinDescriptor` → editor DTOs (Kind==Execution → `Execution bool`, Direction map), List/Search(+category filter)/Get/`GetCompatibleNodes` (drag-direction → opposite-pin data-type assignability, mirrors `visualgraph.assignable`)/`GetTypeHierarchy` (structural `[T, any]`). `editorplugin.go`: `EditorPlugin` (editor.GraphEditorPlugin) — Open/Close → DebugStore, OnNodeSelected merges static node type + runtime stats, OnConnectionChanged reuses the exact ValidateGraph per-connection checks (endpoint/direction/kind/type) as **advisory, never-mutating** pre-commit validation (removals always valid), OnPropertyChanged (unknown prop → warning not error). `debugger.go`: `Debugger` (editor.GraphDebugger) — **non-invasive** model: the interpreter records a trace; the debugger replays it via a cursor (StepOver/Into/Continue) + breakpoints + variable defaults + latest pin values (never pauses live frames). `debug.go`: `DebugStore` — RWMutex-guarded per-graph breakpoints/exec-count/last-error/latest-frame + a bounded (256) trace ring + a drainable `TraceEvent` queue (protocol-free — the `.2` sync system maps events → `pkg/protocol`). **97.4% cov**; default `go test ./...` **74 pkgs green**, vet + modernize + gofmt clean; `-race` in CI (CGO absent locally). — `internal/grapheditor/`
- [x] [T-6P04·rem.2] `graphDebugSyncSystem` + interpreter trace hook *(**Done 2026-06-03** — closes Group 4)* — **Trace hook (`pkg/visualgraph/interpreter.go`):** neutral `ExecutionFrame` DTO + `TraceRecorder` interface (defined *in* visualgraph so it imports no editor/protocol type — the dependency points the right way) + `Interpreter.RunTraced(g, entry, sink, rec)`; `Run` now delegates to `RunTraced(…, nil)` (purely additive, byte-identical — existing DataEval/Deterministic/Branch/StepLimit tests stay green), recorder passed **per-pass** (in `run`, not on the shared Interpreter → concurrency-safe across entities), nil rec = zero overhead. **Bridge (`internal/grapheditor/recorder.go`):** `Recorder` (vg.TraceRecorder) maps `vg.ExecutionFrame` → `editor.GraphExecutionFrame` (stamping wall-clock time) into the DebugStore, bound to one graph+entity; `RecordError` for failed runs. **Sync system (`internal/grapheditor/sync.go`):** `SyncPlugin` (appface.Plugin, opt-in/editor-attached, **not** in DefaultPlugins) registers `grapheditor.DebugSync` in `PostUpdate`; each frame it `DrainEvents()` → maps each `TraceEvent` to the matching **Engine→Editor** `pkg/protocol` message (`GraphRuntimeError` if Err, else `GraphBreakpointHit` if breakpointed, else single-frame `GraphExecutionTraceEvent`) → injected `GraphEventSink` (app wires to the editor transport; tests capture). Drain is unconditional (nil sink still clears the queue — can't grow); `GraphLiveUpdate` is the inbound direction, handled by the IPC receiver not the drain. **Honest scope:** the per-entity "execute graphs each frame" ECS system is separate Track-P runtime work, not the editor bridge — the hook + sync seam are complete and integration-testable end-to-end (RunTraced → DebugStore → sync → captured protocol msgs) for T-6T07. **visualgraph 89.5% / grapheditor 97.6% cov**; default `go test ./...` **75 pkgs green**; vet + modernize + editor build clean; own files gofmt-clean (3 pre-existing CRLF files in pkg/visualgraph untouched — out of scope); `-race` in CI (CGO absent locally). — `pkg/visualgraph/interpreter.go`, `internal/grapheditor/{recorder,sync}.go`

### Validation

- [x] [T-6T07] App-integration validation *(**Done 2026-06-03** — closes the App-integration cohort)* — `examples/editor/` (separate module, `replace … => ../..`, mirrors `examples/diagnostic`/`ui`; added to `go.work`). `run() (uint64, error)` builds **one headless `app.App`** wiring the cohort and hashes only **deterministic facts** (the loop runs on the real wall clock — gametime feeds diagnostics, the live recorder stamps `time.Now` — so metric *values* are never hashed). Validates: **window** (`window.SyncPlugin` headless backend composes + ticks), **diagnostics** (`diag.DiagnosticsPlugin` + reader-gate INV-1), **in-process plugin load** (compile-time factory → `LoadInProcess` → `Build→Ready`, then `FinishInProcess` → `Finish→Cleanup`, asserting `StateActive`→`StateDisabled`), **definition hot-reload** (`pkgdef.Decode` → `InstanceStore.Instantiate` → `Reload` with a modified scene, asserting stale entities are despawned — the operation the hot-reload system runs on an asset `Modified`), **ui** (`ui.Solve` of a layout tree → deterministic rects), **graph-debug round-trip** (seed a fixed-timestamp trace → drive the `PostUpdate` `grapheditor.DebugSync` system through one App frame → 3 `pkg/protocol` messages → each asserted to survive `Encode → Decode` deep-equal, wire bytes folded into the hash). `main_test.go`: `TestEditorHashStable` (≥20-run determinism, C29) + `TestGraphTraceLivePath` (the live `Interpreter.RunTraced → grapheditor.Recorder → DebugStore` path, structure-asserted since its timestamp is wall-clock). **Golden `editor=13803481501758845285`** committed to `examples/goldens.json`; `cmd/examplecheck` reports **17/17 examples OK**. Main module `go test ./...` **75 pkgs green**; example gofmt + vet + modernize clean; `-race` in CI (CGO absent locally). **Honest scope:** the asset-file-watch trigger (dev-build `NewReloadWatcher`) and a real GPU/font UI render are not exercised (headless, T-6X04-gated) — the hot-reload *reload-registry* and UI *layout* are validated, which is the deterministic CI-safe half. — `examples/editor/`
...
- [~] [T-6H02] Transports: **stdio + http done** (`StdioConnection` newline-delimited JSON + `MemConnection` test responder in `transport.go`; `HTTPTransport`/`HTTPConnection` stdlib `net/http` in `http.go` — T-6H02·rem 2026-06-02) + `ContextProvider`; **websocket deferred** (needs a ws-dep ADR, C-003). — `pkg/assistant/{transport,http,context}.go` `[Bootstrap]`
...
- [x] [T-6J02] Compatibility test harness: snapshot of `pkg/` public surface; CI fails on undocumented breaking change. — `cmd/apidiff/`, `api/snapshot.json` `[Bootstrap]`
- **Verify (J01):** SemVer Parse (v-prefix/pre-release strip, partial→0, invalid→`ErrInvalidVersion`); Compare ordering (major>minor>patch); Constraint matrix (^/~/>=/>/<=/</=/bare/range/empty, caret-on-0.x pins minor); bad clause → `ErrInvalidVersion`; `IsGoToolchainSupported` at/above/below `MinGoToolchain` + `go`-prefix + garbage; **`pkg/plugin` Track N tests still green** after the extraction (alias identity preserved).
- **Verify (J02):** `extractFromSource` captures exported func/method/type/const/var (excludes unexported names + methods on unexported receivers; handles generic receivers + type aliases); `Diff` classifies removed/changed (breaking) vs added (minor); identical/add-only diffs non-breaking; snapshot round-trips (missing→empty, malformed→error); `scanPackages` skips `_`/test files; `run` -update writes / clean→0 / removal→1 / `-json` valid.
- **Done (2026-05-31):** Extracted the canonical SemVer kernel to **`pkg/version`** (DRY — Track N's `pkg/plugin` and Track J's engine compat share one implementation): `version.go`/`constraint.go`/`policy.go` (`MinGoToolchain` 1.26.3 + `GoToolchainConstraint`/`IsGoToolchainSupported` dogfooding the parser). Refactored `pkg/plugin/{semver,constraint}.go` to **type-alias re-exports** — public SDK API + Track N tests unchanged. **pkg/version 98.9% cov**.
- **Done (2026-06-01, T-6J02):** `cmd/apidiff` (path reconciled `scripts/api-diff/` → testable Go tool) — snapshots the exported API of `pkg/...` via `go/ast`+`go/printer` (stdlib, C-003; signatures rendered through the printer so reformatting is not a spurious diff), compares to committed `api/snapshot.json`, fails on **removed/changed** exports (breaking) while **added** is non-breaking. `-update` rewrites the snapshot; `-json` for CI. **85.5% cov**; dogfooded `api/snapshot.json` (35 packages, self-compare clean). The fourth member of the "committed snapshot → drift" family (with benchcompare/baseline, examplecheck/goldens, releasenotes/breaking) — `pkg/version` defines what breaking *means*, this *detects* it. `go test ./...` 70 pkgs green, build/modernize clean. **Track J complete.**
...
- [x] [T-6O01] Config struct + `DefaultConfig` + embedded `plugin.toml` + full `New()`/Build/Ready/Finish/Cleanup lifecycle (`appface.FullPlugin`) + `ServiceRegistry` registration. **Done** (config 2026-05-31; lifecycle + manifest + service — T-6O01·rem 2026-06-03). — `pkg/plugins/aiapi/{config,plugin,service,manifest}.go` + `plugin.toml` `[Bootstrap]`
- [x] [T-6O02] `Provider` abstraction + registry + `Select` + canonical request/response types + **all four providers** (OpenAI/local build+parse+native SSE; Anthropic/Gemini one-file each — T-6O05·rem 2026-06-03). — `pkg/plugins/aiapi/{provider,canonical,provider_openai,provider_anthropic,provider_gemini,provider_local,httpclient}.go` `[Bootstrap]`
- [x] [T-6O03] Rate limiter (RPM+TPM token bucket per provider, INV-8) + method dispatch (10 standard methods) + SSE streaming + token/cost diagnostics. **Done** (rate limiter 2026-05-31; SSE+dispatch+diag — T-6O03·rem 2026-06-03). Per-request cancellation map for streaming deferred (the `Dispatcher` honours `ctx` per call; an editor-side cancel-by-request-ID registry lands with the event-bus wiring). — `pkg/plugins/aiapi/{ratelimit,stream,dispatch,diagnostics,provider_openai}.go` `[Bootstrap]`
...
- [x] [T-6O05] `FakeProvider` deterministic responder for CI + `CheckParity`/`RunParity` canonical mode-invariance harness (INV-7 — in-process ≡ OOP JSON wire boundary; full subprocess-RPC parity awaits OOP method dispatch). **Done** (FakeProvider 2026-05-31; parity + providers — T-6O05·rem 2026-06-03). — `pkg/plugins/aiapi/{fake,parity}.go` `[Bootstrap]`
